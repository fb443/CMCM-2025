...
import argparse, csv, math, pickle, os
...
def load_multidigraph(path, time_attr="travel_time_sec"):
    """Load a MultiDiGraph from gpickle and return:
       H: DiGraph with per-arc service time minutes t = min over parallel edges
       eid_uv: dict eid->(u,v) using the SAME enumeration scheme as in the MIP
    """
    # NetworkX 3+ safe load
    try:
    ...
    # Aggregate parallel arcs to a simple DiGraph for deadhead + service time
    H = nx.DiGraph()
    # Also reconstruct the MIP's eid assignment order
    eid_uv = {}
    eid = 0
    for u, v, k, d in G.edges(keys=True, data=True):
        # seconds → minutes; fallback to 60 sec if missing
        tt_sec = d.get(time_attr, d.get("weight", 60.0))
        tt_min = max(1e-3, float(tt_sec) / 60.0)
        # keep minimum service time for (u,v)
        if H.has_edge(u, v):
            if tt_min < H[u][v]["t"]:
                H[u][v]["t"] = tt_min
        else:
            H.add_edge(u, v, t=tt_min)
        # eid mapping as done in the MIP (linear enumeration over edges(keys=True,data=True))
...
def load_routes_from_pickles(pkl_paths: List[str]) -> Tuple[Dict[int, List[Tuple[int,int]]], Dict[int, float]]:
    """
    Returns merged (routes_uv, f_by_route) with fresh route ids 0..K-1.
    Accepts pickles shaped like:
      {"routes_uv": {k:[(u,v),...], ...}, "f_by_route": {k:cycle_min, ...}, ...}
    """
    ...
    for pth in pkl_paths:
        ...
        if isinstance(obj, dict):
            ...
            # fallback: routes as list-of-lists of (u,v)
            routes = obj[1]
        ...
        for ok in sorted(routes_uv.keys()):
            ...
            # default cycle if missing; will be replaced by CLI default later in build_jobs
...
def ordered_walk_from_arcs(arcs):
    """Eulerian-like order for a given multiset list of (u,v)."""
    if not arcs: return [], None, None
    G = nx.DiGraph()
    for u, v in arcs:
        if G.has_edge(u, v): G[u][v]["multi"] += 1
        else: G.add_edge(u, v, multi=1)
    outd, ind = G.out_degree(), G.in_degree()
    start = next((n for n in G if outd[n]-ind[n]==1), None)
    if start is None: start = next((n for n in G if outd[n]>0), None)
    succ = {u: deque([v for v in G.successors(u) for _ in range(G[u][v]["multi"])]) for u in G.nodes()}
    stack, trail = [start], []
    while stack:
        u = stack[-1]
...
# ---------- Distances ----------
def all_pairs_shortest_time(H):
    """dict-of-dicts with minutes using edge attr 't'."""
    return {u: nx.single_source_dijkstra_path_length(H, u, weight="t") for u in H.nodes()}

# ---------- Job expansion ----------
def build_jobs(G, routes_uv, f_by_route, default_cycle, horizon_M):
    route_meta = {}
    for k, uv in routes_uv.items():
        ordered, s_node, e_node = ordered_walk_from_arcs(uv)
        svc = sum(G.edges[u, v]["t"] for (u, v) in uv)
        fk = float(f_by_route.get(k, default_cycle))  # use default if missing/NaN
        if not math.isfinite(fk):
            fk = float(default_cycle)
        if svc > fk + 1e-9:
            raise ValueError(f"Route {k}: service {svc:.2f} > cycle {fk:.2f} → infeasible.")
    ...
    for k, meta in route_meta.items():
        ...
        for p in range(copies):
            rel = p * fk
            due = min((p + 1) * fk, horizon_M)  # end-of-service window upper bound
            # end-of-service window: [rel + svc, rel + fk]
            if rel + svc <= due + 1e-9:
                jobs.append(dict(id=len(jobs), k=k, p=p, proc=svc, rel=rel, due=due))
    return route_meta, jobs

# ---------- OR-Tools scheduling (Routing) ----------
def solve_schedule(H, route_meta, jobs, horizon_M, fleet_max=None, vehicle_fixed_cost=10000, allow_wait=True):
    if fleet_max is None:
        fleet_max = max(1, len(jobs))
    #print("allow_wait",allow_wait)
    #print("num jobs: ", jobs)
    #print("fleet max: ", fleet_max)
    N = len(jobs)
    depot = N
    ...
    rel        = [int(round(j["rel"])) for j in jobs]
    due        = [int(round(j["due"])) for j in jobs]
    ...
    routing.AddDimension(
        transit,
        slack_max=slack,
        capacity=int(horizon_M),
        fix_start_cumul_to_zero=True,
        name="Time",
    )
    time_dim = routing.GetDimensionOrDie("Time")

    # AFTER: end-of-service windows
    for j in range(N):
        idx = manager.NodeToIndex(j)
        k = jobs[j]["k"]
        cycle_k = route_meta[k]["cycle"]
        rel_end = int(round(jobs[j]["rel"] + svc_time[j]))   # earliest feasible end
        due_end = int(round(jobs[j]["rel"] + cycle_k))       # must finish by period end
        # guard against rounding pathologies
        if rel_end > due_end:
            rel_end = due_end  # gives an instant window; if that happens often, stop rounding to int
        time_dim.CumulVar(idx).SetRange(rel_end, due_end)


    for v in range(fleet_max):
        time_dim.CumulVar(routing.Start(v)).SetRange(0, int(horizon_M))
        time_dim.CumulVar(routing.End(v)).SetRange(0, int(horizon_M))
...
def main():
    ...
    # if your graph input is now a gpickle (MultiDiGraph), keep that loader you added earlier
    ...
    ap.add_argument("--pkls", nargs="+", required=True, help="One or more route pickles to merge")
    ...
    # load graph from gpickle the same way you already implemented
    G = load_multidigraph(args.graph, args.graph_format)  # use your existing loader
    # collapse to DiGraph with weight "t" (minutes) exactly as in your current code
    # (assumes you already do: pick min travel_time across parallel edges and set edge attr "t")

    routes_uv, f_by_route = load_routes_from_pickles(args.pkls)
    if not routes_uv:
        raise SystemExit("No routes in provided pickles.")

    route_meta, jobs = build_jobs(G, routes_uv, f_by_route, args.default_cycle, args.horizon)
    schedule, used = solve_schedule(
        G, route_meta, jobs, args.horizon, args.fleet_max, int(round(args.veh_fixed_cost))
    )
    print(f"Used trucks: {used} / max {args.fleet_max or len(jobs)}")
